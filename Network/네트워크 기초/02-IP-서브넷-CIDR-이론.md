...
| 서브넷 | Subnet | 접두사 길이(또는 서브넷 마스크)로 구분한 하나의 IP 주소 범위. 예: `192.168.0.0/24`은 `192.168.0.1`~`192.168.0.254` 장비가 속할 수 있는 하나의 서브넷 |
...
| 네트워크 주소 | Network Address | 호스트 비트가 모두 0인, 서브넷 자체를 가리키는 주소. 예: `192.168.0.85/24`의 네트워크 주소는 `192.168.0.0`이며, 일반적인 IPv4 서브넷에서는 장비에 할당하지 않는다 |
